...
옷장 목록은 페이지 진입 시 `/api/wardrobe/`의 GET으로 DB에서 읽고, 옷 추가는 같은 경로의 POST로 저장합니다. 두 요청 모두 `AGENTRIA_WARDROBE_API_URL` 하나를 사용합니다. 통합 Ability는 `Name`이 비어 있으면 DB 목록만 조회하고, `Name`과 `category`가 있으면 분석·저장 후 DB의 최신 전체 목록을 반환해야 합니다. 두 어빌리티가 같은 API 키를 사용하면 `AGENTRIA_WARDROBE_API_KEY`는 비워둘 수 있습니다.
